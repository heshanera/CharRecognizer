...
int Trainer::changeInputMatrix(int inputMatrixNo){
    
    int charPointerStarter = inputMatrixNo*6;
    int w = 40, h = 40; // width x height of a char (in pixels)
    int size = 1600; // width x height
    ...
    targetChars = new char[chars];
    for (int i = 0; i < chars; i++){
        targetChars[i] = caps[typeOfTrainingChars[i+charPointerStarter]-1];
    }
    ...
    for (int i = 0; i < chars; i++) {
        imgPrc.initializeImage(trainingImages[i+charPointerStarter]);
        imgPrc.createCropedMatrix();
        tmpData2 = imgPrc.resizeImage();        
        tmpData = imgPrc.skeletonize();
